...
# Inicializar NVML
nvmlInit()
handle = nvmlDeviceGetHandleByIndex(0)  # Supondo que esteja usando a GPU 0

def get_gpu_memory_usage():
    info = nvmlDeviceGetMemoryInfo(handle)
    return info.used / 1024 / 1024  # Convertido para MB

def print_gpu_memory():
    if torch.cuda.is_available():
        allocated = torch.cuda.memory_allocated() / (1024 ** 2)  # Em MB
        reserved = torch.cuda.memory_reserved() / (1024 ** 2)  # Em MB
        print(f"Memória GPU alocada: {allocated:.2f} MB, reservada: {reserved:.2f} MB")
    else:
        print("GPU não está disponível.")

def main(opt):
    wandb.init(project="few-shot-learning", entity="gabriel-lacerda-universidade-federal-de-campina-grande")
    ...
    if opt['data.trainval']:
        data, _ = data_utils.load(opt, ['trainval'])
        train_loader = data['trainval']
        val_loader = None
    else:
        data, _ = data_utils.load(opt, ['train', 'val'])
    ...
    def on_start_epoch(state):
        print_gpu_memory()  # Monitora o uso de memória da GPU no início de cada época
        wandb.log({"gpu_memory_usage_mb": get_gpu_memory_usage()})  # Logar uso de memória no W&B
    ...
    def on_update(state):
        print_gpu_memory()  # Monitora o uso de memória da GPU durante as atualizações
        wandb.log({"gpu_memory_usage_mb": get_gpu_memory_usage()})  # Logar uso de memória no W&B
    ...
    def on_end_epoch(hook_state, state):
        ...
        # Registra as métricas no W&B
        wandb.log(meter_vals['train'])
        if val_loader is not None:
            wandb.log(meter_vals['val'])
